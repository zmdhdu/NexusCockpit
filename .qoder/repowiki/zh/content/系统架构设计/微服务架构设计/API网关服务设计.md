...
## 架构总览
整体数据流：客户端HTTP/WS请求进入网关，经鉴权与限流后，由路由器选择目标服务；代理层将HTTP请求转换为gRPC调用，读取后端响应并返回；WebSocket通过Hub维持双向通道。
...
## 结论
本网关以“轻量、可插拔、高可用”为目标，围绕HTTP到gRPC的协议转换为核心，结合鉴权、限流、缓存、路由与服务发现，形成统一的API接入面。通过合理的超时、重试与熔断策略保障稳定性，并通过完善的监控指标体系支撑运维与优化。
