...
## v2.1 增强功能（2026-07-12）

- **Agent 工具准确性**：新增位置查询能力，支持"我在哪"等指令
- **音乐播放**：后端提供 10 首 Demo 音频，前端 HTML5 Audio 实际播放
- **TTS 语音合成**：基于浏览器 Web Speech API，回复后自动语音播报
- **运营总览实时更新**：每次对话自动记录指标到 Redis，看板数据即时刷新
- **用户习惯持久化**：`user_habits` 表存储用户偏好，跨座舱切换不丢失
- **隐私数据隔离**：`chat_logs` 表存储聊天记录，管理员仅可见聚合指标
- **系统监控增强**：新增 LLM/TTS/ASR/应用配置/OSS 五类配置监控
