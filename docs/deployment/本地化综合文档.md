...
| # | 问题描述 | 位置 | 修复方案 | 完成日期 | Git Status 标记 |
|---|----------|------|----------|----------|----------------|
| P0-1 | JWT 默认弱密钥 `change-me-in-production` | `backend_design/nexus/config.py:283-285` | 生产模式弱密钥直接 `raise ValueError` | 2026-07-30 | M config.py |
| P0-2 | `.env` 文件被 git 跟踪 | 仓库根目录 | `git rm --cached .env` + 清理历史 | 2026-07-30 | M .gitignore, .env.example |
| P0-3 | CORS 默认 `["*"]` 全开 | `backend_design/nexus/config.py:424-426` | 生产环境读取白名单，禁止通配符 | 2026-07-30 | M config.py |
| P0-4 | WebSocket `CheckOrigin` 恒返回 true | `nexus_gate/internal/ws/hub.go:22-24` | 添加 Origin 白名单校验 | 2026-07-30 | M hub.go |
| P0-5 | Token 签发缺凭证校验 | `nexus_gate/internal/router/router.go:261-262` | 所有角色统一密码验证或声纹绑定 | 2026-07-30 | M router.go |
| P0-6 | docker-compose 硬编码密码 | `docker-compose.yml:114-115` | 改用 `${VAR}` 注入 | 2026-07-30 | M docker-compose.yml |
| P0-7 | CI 全部步骤 `\|\| true` 不阻断 | `.github/workflows/ci.yml:25-49` | 移除 `\|\| true`,严格阻断 | 2026-07-30 | M ci.yml |

**详细修复方案**: 参考本文档第九章"技术实现细节"。
