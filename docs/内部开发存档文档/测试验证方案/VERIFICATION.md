...
9. [阶段 9: 修复验证](#阶段-9-v230-修复验证)
10. [附录: 常见问题排查](#附录-常见问题排查)
...
| # | 输入指令 | 预期结果 | 验证点 |
|---|----------|----------|--------|
| 1 | "打开空调" | 空调已开启 | power_on 不再提前 return |
| 2 | "打开空调温度22度" | 空调已开启，温度已设为22度 | power_on + target_temp 同时生效 |
| 3 | "打开空调温度22度风速1" | 空调已开启，温度22度，风量1档 | power_on + target_temp + fan_speed 同时生效 |
| 4 | "关闭空调" | 空调已关闭 | power_off 正常 |
| 5 | "空调调到24度" | 温度已设为24度 | 仅参数设置无电源操作 |
| 6 | "空调温度调高" | 温度+1度 | temp_up 微调 |
| 7 | "空调风速8" | 风量超出范围警告 | sandbox 参数校验 |

### 9.2 多需求并行调度验证

| # | 输入指令 | 预期结果 | 验证点 |
|---|----------|----------|--------|
| 1 | "打开音乐，关闭车窗" | 音乐播放 + 车窗关闭 | 跨域多动作并行执行 |
| 2 | "空调调到24度，打开音乐" | 空调设置 + 音乐播放 | climate + media 并行 |
| 3 | "关闭车窗，空调开到26度，调座椅" | 车窗关闭 + 空调开启 + 座椅调整 | 三动作并行 |
| 4 | "打开车窗，关闭车窗" | 串行执行（互斥组） | window 互斥组串行 |
| 5 | "打开音乐，关闭车窗，空调调到24度，导航到公司" | 全部需求处理 | 跨专家多需求 |

### 9.3 意图拆解跨域误匹配验证

| # | 输入指令 | 预期结果 | 验证点 |
|---|----------|----------|--------|
| 1 | "打开音乐，关闭车窗" | Media: play + Window: close | "打开"不误匹配为车窗 open |
| 2 | "关闭空调，打开车窗" | Climate: off + Window: open | "关闭"不误匹配为车窗 close |
| 3 | "空调开到27度" | Climate: set_temp 27 | "开到"不误匹配为 Navigation_Action |

### 9.4 路由防漂移验证

| # | 输入指令 | 预期结果 | 验证点 |
|---|----------|----------|--------|
| 1 | "空调开到27度" | 路由到 vehicle 专家 | Navigation_Action 无 destination 时不触发导航 |
| 2 | "导航到公司" | 路由到 navigation 专家 | 正常导航路由 |
| 3 | "打开空调，导航到公司" | 路由到 vehicle + navigation | 多专家并行 |

### 9.5 参数校验验证

| # | 输入指令 | 预期结果 | 验证点 |
|---|----------|----------|--------|
| 1 | "空调温度0度" | 拒绝执行 | 极端温度阻断 |
| 2 | "空调温度100度" | 拒绝执行 | 极端温度阻断 |
| 3 | "车窗百分之-50" | 拒绝执行 | 极端百分比阻断 |
| 4 | "车窗位置xxx" | 回退到 all | 非法位置回退 |
| 5 | "空调操作xxx" | 拒绝执行 | 非法操作符阻断 |

### 9.6 异常兜底验证

| # | 场景 | 预期结果 | 验证点 |
|---|------|----------|--------|
| 1 | 车控指令执行超时(>10s) | "车控指令执行超时，设备可能离线" | asyncio.wait_for 超时捕获 |
| 2 | registry.execute 抛异常 | "车控指令执行异常: ..." | Exception 捕获 |
| 3 | 专家返回空回复 | "指令已执行，但未返回详细信息。" | Responder 空回复兜底 |
