...
| 用例 | 测试内容 | 输入 | 预期结果 |
|------|----------|------|----------|
| `test_climate_set_temp` | 设置空调温度 | `op=set_temp, target_temp=24` | 温度=24, 消息含"24" |
| `test_climate_temp_up` | 升高温度 | `op=temp_up` | 温度+1 |
| `test_window_open` | 打开所有车窗 | `op=open, position=all` | all=100 |
| `test_window_close` | 关闭左前窗 | `op=close, position=front_left` | front_left=0 |
...
| `test_vehicle_status` | 查询车辆状态 | 无参数 | 消息含"胎压" |
...
#### 2. HeuristicRouter 测试 (启发式意图路由)

| 用例 | 输入文本 | 预期意图 |
|------|----------|----------|
| `test_climate_route` | "把空调调到24度" | Climate_Action.target_temp=24 |
| `test_window_route` | "打开车窗" | Window_Action.op=open |
| `test_navigation_route` | "导航到上海虹桥火车站" | Navigation_Action.destination含"上海虹桥" |
| `test_media_route` | "播放音乐" | Media_Action.op=play |
| `test_no_match` | "今天天气真好" | 空字典 {} |

#### 3. SkillRegistry 测试 (技能注册中心)

| 用例 | 测试内容 | 预期结果 |
|------|----------|----------|
| `test_list_skills` | 列出所有技能 | 包含 vehicle_climate, web_search, order_food |
| `test_get_all_tools` | 获取 Tool Schema | 数量 >= 9 |
| `test_execute_climate` | 执行空调技能 | status=ok, action=vehicle_climate |
| `test_execute_unknown` | 执行不存在技能 | status=error |

#### 4. AgentState 测试 (Agent 状态模型)

| 用例 | 测试内容 | 预期结果 |
|------|----------|----------|
| `test_default_state` | 默认状态 | user_input="", intent={} |
| `test_custom_state` | 自定义状态 | 正确存储输入值 |
...
> **注意**: 集成测试需要后端服务可启动，但不需要外部中间件 (Milvus/Redis 等) 连接成功。

### 测试用例详情

| 用例 | API | 方法 | 路径 | 预期 |
|------|-----|------|------|------|
| `test_root` | 根路径 | GET | `/` | 200, name=NexusCockpit |
| `test_health` | 健康检查 | GET | `/health` | 200, 含 status 字段 |
| `test_admin_skills` | 技能列表 | GET | `/admin/skills` | 200, count >= 0 |
| `test_vehicle_status` | 车辆状态 | GET | `/vehicle/status` | 200, 含 success |
...
前端不需要单独运行测试，通过浏览器访问验证:

| 页面 | URL | 验证点 |
|------|-----|--------|
| 仪表盘 | http://localhost:3000/dashboard | 统计卡片显示、服务状态、缓存统计 |
| 语音助手 | http://localhost:3000/chat | 聊天界面、输入框、流式响应 |
| 车控面板 | http://localhost:3000/vehicle | 6 个控制卡片、按钮点击响应 |
| 设置 | http://localhost:3000/settings | API 密钥、模型配置、数据库状态 |
