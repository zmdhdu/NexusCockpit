...
|---|----------|------|----------|----------|----------------|----------|
|  | JWT 默认弱密钥 `change-me-in-production` | `backend_design/nexus/config.py:L729-731` | 生产模式弱密钥直接 `raise ValueError` | 2026-07-31 | M config.py | ✅ 代码审计 |
|  | `.env` 文件被 git 跟踪 | 仓库根目录 | `git rm --cached .env` + 清理历史 | 2026-07-31 | M .gitignore, .env.example | ✅ Git Status 确认 |
|  | CORS 默认 `[\*]` 全开 | `backend_design/nexus/config.py:L732-734` | 生产环境读取白名单，禁止通配符 | 2026-07-31 | M config.py | ✅ 代码审计 |
|  | WebSocket `CheckOrigin` 恒返回 true | `nexus_gate/internal/ws/hub.go:L23-37` | 添加 Origin 白名单校验 | 2026-07-31 | hub.go | ✅ 代码审计已实现 |
|  | Token 签发缺凭证校验 | `nexus_gate/internal/router/router.go:L301-356` | 所有角色统一密码验证或声纹绑定 | 2026-07-31 | router.go | ✅ 代码审计已实现 |
|  | docker-compose 硬编码密码 | `docker-compose.yml:114-115` | 改用 `${VAR}` 注入 | 已完成 | M docker-compose.yml | ✅ 已修复为 ${VAR:-default} 模式 |
|  | CI 全部步骤 `\|\| true` 不阻断 | `.github/workflows/ci.yml:25-49` | 移除 `\|\| true`,严格阻断 | 待执行 | ci.yml | ⏳ 需验证 workflow |
...
| 读者需求 | 推荐文档 |
|----------|----------|
| **快速上手** | [`README.md`](../../README.md)、[`docs/交付版文档包/08-系统架构总览.md`](../交付版文档包/08-系统架构总览.md) |
| **部署运维** | [`docs/交付版文档包/01-部署安装手册.md`](../交付版文档包/01-部署安装手册.md)、[`docs/内部开发存档文档/测试验证方案/VERIFICATION.md`](../测试验证方案/VERIFICATION.md) |
| **架构设计** | [`docs/内部开发存档文档/架构详细设计/README.md`](../架构详细设计/README.md) |
...
| 版本 | 日期 | 主要变更 |
|------|------|----------|
|  | 2026-07-30 | 初始版本，包含 P0 安全加固完整记录 |
|  | 2026-07-31 | 合并改进计划/实施指南/执行总结，消除文档冗余 |
...
- **[docs/内部开发存档文档/架构详细设计/](../架构详细设计/)** - 7 层架构文档
...
**版本记录**:
-  (2026-07-31): 初始版本，整合了所有分散的本地化改造文档
