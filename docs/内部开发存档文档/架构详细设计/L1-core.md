...
> 对应代码: `nexus/config/__init__.py` + `nexus/core/`
...
- **熔断器** — 保护云端 API 调用的稳定性
- **JWT 认证** — 令牌签发、验证、依赖注入
- **个性化服务** — 新增，声纹识别+偏好匹配+Prompt 注入

> **简化**: OSS 对象存储模块 (`core/oss.py`) 已移除（未集成，过度设计）。如需对象存储可参考架构详细设计文档中的存储方案设计。
...
> **简化**: `RabbitMQConfig` 已移除（Celery/RabbitMQ 未落地，任务队列改为 `asyncio.create_task`）。
...
### core/personalization.py — 个性化服务 (新增)
