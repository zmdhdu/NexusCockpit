...
仅在 LLM 降级模式启用时需要。默认使用云端 API，无需下载。

```bash
# 从 HuggingFace 下载 Qwen3.5-4B (GGUF 量化版)
# (需要安装 huggingface_hub)
pip install huggingface_hub

huggingface-cli download Qwen/Qwen3.5-4B-Instruct-GGUF --local-dir ./models/llm/qwen
```
...
项目根目录已包含 `.env` 默认配置文件，开箱即用。如需覆盖默认值，创建 `.env.local`：
...
| `LLM_FALLBACK_ENABLED` | 是否启用本地 LLM 降级 | `false` (使用云端 API) |
...
### Q: Docker Compose 启动后 Milvus 一直 restarting

```bash
# Milvus 首次启动需要 30-60 秒初始化 etcd 和 minio
# 如果超过 2 分钟仍未就绪：
...
### Q: 后端启动报 `ModuleNotFoundError: No module named 'nexus'`

```bash
# 必须在 backend_design 目录下执行启动命令
cd backend_design
python -m nexus.main
...
### Q: Neo4j 连接失败 (端口 17687)

```bash
# 1. 确认 Neo4j 容器已启动
docker compose ps neo4j

# 2. 等待 Neo4j 初始化完成（约 15 秒）
docker compose logs neo4j | tail -5
# 看到 "Bolt enabled on 0.0.0.0:7687" 表示就绪

# 3. 验证密码与 .env 中 NEO4J_PASSWORD 一致
```
