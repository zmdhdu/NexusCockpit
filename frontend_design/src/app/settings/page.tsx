...
export default function SettingsPage() {
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      // 检查 success 字段，后端模型不可用时返回 503 会走 catch
      if (result.success === false) {
        toast.error("声纹注册失败", {
          description: result.message || "声纹识别服务不可用",
        });
      } else {
      ...
      // 后端返回 503 时 axios 抛出错误，提取 detail 信息
      const detail = e?.response?.data?.detail || e?.message || "请确保音频清晰且时长不少于3秒";
      toast.error("声纹注册失败", {
        description: detail,
      });
    } finally {
        ...
        >
          <Card className="glass h-full">
            ...
            <CardContent className="space-y-3">
              ...
              {/* 录音功能（降级方案） */}
              <div className="space-y-2">
                <VoiceRecorder onRecordingComplete={setEnrollFile} />
              </div>

              {/* 或上传文件 */}
              <div className="flex items-center gap-2 text-center text-sm before:flex-1 before:border-t before:border-border after:flex-1 after:border-t after:border-border">
