# 美团分销领券 Skill 诊断手册（DOCTOR.md）

> 本文档是 SKILL.md 的附属诊断模块，仅在用户明确说「分销诊断」「分销排查」「fenxiao doctor」时执行。
...
python3 -c "
import httpx, json
url = 'https://media.meituan.com/fulishemini/couponActivity/aiSendCouponDistribution'
try:
...
```bash
python3 ~/.catpaw/skills/meituan-fenxiao-promotion-coupon/scripts/auth.py token-verify
```
...
日志路径由脚本内部 `tempfile.gettempdir()` 决定，跨平台一致。

按接口分类展示，每个接口取最新一条：

```bash
python3 ~/.catpaw/skills/meituan-fenxiao-promotion-coupon/scripts/diag_auth_log.py
```
...
```bash
python3 ~/.catpaw/skills/meituan-fenxiao-promotion-coupon/scripts/diag_issue_log.py
```
...
### A6 发券接口实时调用（仅 Token 有效时执行）

先获取 token：
```bash
python3 -c "import json; from pathlib import Path; t = json.loads((Path.home()/'.xiaomei-workspace/auth_tokens.json').read_text()); print(t.get('user_token',''))"
```

再执行发券：
```bash
python3 ~/.catpaw/skills/meituan-fenxiao-promotion-coupon/scripts/issue.py --token <上一步获取的token>
```
...
```bash
python3 -c "import json; from pathlib import Path; config = json.loads((Path.home()/'.catpaw/skills/meituan-fenxiao-promotion-coupon/scripts/config.json').read_text()); ai_scene = config.get('aiScene', ''); masked = (ai_scene[:6] + '****' + ai_scene[-6:]) if len(ai_scene) > 12 else ai_scene; print(json.dumps({'aiScene': masked, 'exists': bool(ai_scene)}, ensure_ascii=False))"
```
